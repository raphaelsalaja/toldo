<ComponentContainer>
  <ShowcaseExample/>
</ComponentContainer>

# Introduction

Toldo is an elevated component built on top of <Code>@radix-ui</Code> that provides a few extra features to make it easier to work with dialogs in your application.

# Installation

Install the component from your command line.

```bash title="terminal"
pnpm install toldo/dialog
```

# Anatomy

Import all parts and piece them together.

```tsx title="dialog.tsx"
import * as Dialog from "@radix-ui/react-dialog";

export default () => (
  <Dialog.Root>
    <Dialog.Trigger />
    <Dialog.Portal>
      <Dialog.Overlay />
      <Dialog.Content>
        <Dialog.Title />
        <Dialog.Description />
        <Dialog.Close />
      </Dialog.Content>
      <Dialog.Stack>
        <Dialog.StackContent>
          <Dialog.StackTitle />
          <Dialog.StackDescription />
          <Dialog.Button />
        </Dialog.StackContent>
      </Dialog.Stack>
    </Dialog.Portal>
  </Dialog.Root>
);
```

# API Reference

# Root

Contains all the parts of a dialog. 

<PropsTable
	data={[
		{
			name: "defaultOpen",
			nameDetails: (
				<span>
					The open state of the dialog when it is initially rendered. Use when
					you do not need to control its open state.
				</span>
			),
      type: "boolean",
		},
		{
			name: "open",
			nameDetails: (
				<span>
					The controlled open state of the dialog. Must be used in conjunction
					with <Code>onOpenChange</Code>.
				</span>
			),
      type: "boolean",
		},
		{
			name: "onOpenChange",
      nameDetails: (
				<span>
					Event handler called when the open state of the dialog changes.
				</span>
			),
			type: "function",
      typeDetails: "(open: boolean) => void",
		},
		{
			name: "modal",
			nameDetails: (
				<span>
					The modality of the dialog. When set to <Code>true</Code>, interaction
					with outside elements will be disabled and only dialog content will be
					visible to screen readers.
				</span>
			),
      type: "boolean",
      default: "true",
		},
      {
      name: "dialogs",
      nameDetails: "The dialogs to render.",
      type: "array",
      typeDetails: "Array<{id: string, dialog: React.ReactNode}>",
    },
	]}
/>

# Trigger

The button that opens the dialog.

<PropsTable
	data={[
		{
			name: "asChild",
			nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
			type: "boolean",
			default: "false",
		},
	]}
/>

# Portal

When used, portals your overlay and content parts into the <Code>body</Code> element.

<PropsTable
	data={[
		{
			name: "forceMount",
			nameDetails: (
				<span>
					Used to force mounting when more control is needed. Useful when
					controlling animation with React animation libraries. If used on this
					part, it will be inherited by <Code>Dialog.Overlay</Code> and
					<Code>Dialog.Content</Code>.
				</span>
			),
			type: "boolean",
		},
		{
			name: "container",
			nameDetails: "Specify a container element to portal the content into.",
			type: "HTMLElement",
			default: "document.body",
		},
	]}
/>

# Overlay

A layer that covers the inert portion of the view when the dialog is open.

<PropsTable
	data={[
		{
			name: "asChild",
			nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
			type: "boolean",
			default: "false",
		},
		{
			name: "forceMount",
			nameDetails: (
				<span>
					Used to force mounting when more control is needed. Useful when
					controlling animation with React animation libraries. It inherits from
					<Code>Dialog.Portal</Code>.
				</span>
			),
			type: "boolean",
		},
	]}
/>

# Content

Contains content to be rendered in the open dialog.

<PropsTable
	data={[
		{
			name: "asChild",
			nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
      type: "boolean",
			default: "false",
		},
		{
			name: "forceMount",
			type: "boolean",
			nameDetails: (
				<span>
					Used to force mounting when more control is needed. Useful when
					controlling animation with React animation libraries. It inherits from
					<Code>Dialog.Portal</Code>.
				</span>
			),
		},
		{
			name: "onOpenAutoFocus",
      nameDetails: (
				<span>
					Event handler called when focus moves into the component after
					opening. It can be prevented by calling
					<Code>event.preventDefault</Code>.
				</span>
			),
			type: "function",
			typeDetails: "(event: Event) => void",
		},
		{
			name: "onCloseAutoFocus",
      nameDetails: (
				<span>
					Event handler called when focus moves to the trigger after closing. It
					can be prevented by calling <Code>event.preventDefault</Code>.
				</span>
			),
			type: "function",
			typeDetails: "(event: Event) => void",
		},
		{
			name: "onEscapeKeyDown",
      nameDetails: (
				<span>
					Event handler called when the escape key is down. It can be prevented
					by calling <Code>event.preventDefault</Code>.
				</span>
			),
			type: "function",
			typeDetails: "(event: KeyboardEvent) => void",
		},
		{
			name: "onPointerDownOutside",
      nameDetails: (
				<span>
					Event handler called when a pointer event occurs outside the bounds of
					the component. It can be prevented by calling
					<Code>event.preventDefault</Code>.
				</span>
			),
			type: "function",
			typeDetails: "(event: PointerDownOutsideEvent) => void",
		},
		{
			name: "onInteractOutside",
      nameDetails: (
				<span>
					Event handler called when an interaction (pointer or focus event)
					happens outside the bounds of the component. It can be prevented by
					calling <Code>event.preventDefault</Code>.
				</span>
			),
			type: "function",
			typeDetails: "(event: React.FocusEvent | MouseEvent | TouchEvent) => void",
		},
	]}
/>

# Close

The button that closes the dialog.

<PropsTable
	data={[
		{
			name: "asChild",
      nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
			type: "boolean",
			default: "false",
		},
	]}
/>

# Title

An accessible title to be announced when the dialog is opened.

<PropsTable
	data={[
		{
			name: "asChild",
			nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
      type: "boolean",
			default: "false",
		},
	]}
/>

# Description

An optional accessible description to be announced when the dialog is opened.

<PropsTable
	data={[
		{
			name: "asChild",
      nameDetails: (
				<>
					Change the default rendered element for the one passed as a child,
					merging their props and behavior.
				</>
			),
			type: "boolean",
			default: "false",
		},
	]}
/>

# Stack

A wrapper that holds the dialogs in a stack.

<PropsTable
	data={[
		 {
      name: "offsetY",
      nameDetails: (
        <span>
          The value added to the y-axis of the dialog content, when further down the stack.
        </span>
      ),
      type: "number",
      default: "24",
    },
    {
      name: "offsetScale",
      nameDetails: (
        <span>
          The value added to the scale of the dialog content, when further down the stack.
        </span>
      ),
      type: "number",
      default: "0.05",
    },
    {
      name: "offsetOpacity",
      nameDetails: (
        <span>
          The value added to the opacity of the dialog content, when further down the stack.
        </span>
      ),
      type: "number",
      default: "0.33",
    },
    {
      name: "initial",
      nameDetails: (
        <span>
          The initial animation state of the dialog content.
        </span>
      ),
      type: "object",
      typeDetails: (
         <span>
            <a href="https://www.framer.com/motion/animation/" target="_blank" rel="noreferrer">Motion Props</a>
         </span>
      )
    },
    {
      name: "animate",
      nameDetails: (
        <span>
          The animation state of the dialog content.
        </span>
      ),
      type: "string",
      typeDetails: (
         <span>
            <a href="https://www.framer.com/motion/animation/" target="_blank" rel="noreferrer">Motion Props</a>
         </span>
      )
    },
    {
      name: "exit",
      nameDetails: (
        <span>
          The exit animation state of the dialog content.
        </span>
      ),
      type: "string",
      typeDetails: (
         <span>
            <a href="https://www.framer.com/motion/animation/" target="_blank" rel="noreferrer">Motion Props</a>
         </span>
      )
    },
    {
      name: "transition",
      nameDetails: (
        <span>
          The transition animation state of the dialog content.
        </span>
      ),
      type: "string",
      typeDetails: (
         <span>
            <a href="https://www.framer.com/motion/animation/" target="_blank" rel="noreferrer">Motion Props</a>
         </span>
      )
    },
	]}
/>

# Stack Content
 
Contains content to be rendered in the open dialog within a stack. See [Content](#content) for more details.

<PropsTable data={[]}/>

# Stack Title

An accessible title to be announced when the dialog is opened within a stack. See [Title](#title) for more details.

<PropsTable data={[]}/>

# Stack Description

An optional accessible description to be announced when the dialog is opened within a stack. See [Description](#description) for more details.

<PropsTable data={[]}/>

# Stack Remove

The button that removes the dialog from the stack.

<PropsTable
	data={[
		{
			name: "dialogId",
      nameDetails: (
        <span>
          The id of the dialog to remove from the stack.
        </span>
      ),
      type: "string",
    },
  ]}
/>

# Stack Add

The button that adds a dialog to the stack.

<PropsTable
	data={[
		{
      name: "dialogId",
      nameDetails: (
        <span>
          The id of the dialog to add to the stack.
        </span>
      ),
      type: "string",
    },
  ]}
/>

# Shared Item

A handy component to share animations between dialogs. Wraps [Framer Motion's](https://www.framer.com/motion/layout-animations/) <Code>motion.div</Code> layout into a more readable and accessible component.

<PropsTable
	data={[
		{
			name: "layout",
      nameDetails: (
        <span>
          If <Code>true</Code>, this component will automatically animate to its new position when
          its layout changes. 
        </span>
      ),
      type: "enum",
      typeDetails: "boolean | 'position' | 'size' | 'preserve-aspect'",
    },
    {
      name: "layoutId",
      nameDetails: (
        <span>
          Enable shared layout transitions between different components with the same <Code>layoutId</Code>.
        </span>
      ),
      type: "string",
    },
    {
      name: "onLayoutAnimationStart",
      nameDetails: (
        <span>
          A callback that will fire when a layout animation on this component starts.
        </span>
      ),
      type: "function",
      typeDetails: "() => void",
    },
    {
      name: "onLayoutAnimationComplete",
      nameDetails: (
        <span>
          A callback that will fire when a layout animation on this component completes.
        </span>
      ),
      type: "function",
      typeDetails: "() => void",
    },
  ]}
/>